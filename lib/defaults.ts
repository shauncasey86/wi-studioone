// Single source of truth for the site's default content — the exact copy from
// legacy/studioone.html. Used by the seed (prisma/seed.ts) and by the admin
// "reset to defaults" actions (lib/admin/reset.ts). Plain data, no imports, so
// it loads cleanly under both tsx (the seed runner) and Next.
//
// Rich fields use the *italic* / **bold** / [label](href) convention
// (see lib/richtext). NB is a non-breaking space.
...
export const defaultHeroEyebrows = [
  "Sutton Village · Hull",
  "One room",
  "Est. 2026",
];

export const defaultManifestoFoots = [
  {
    term: "Where",
...
// Reorderable lists, keyed by the admin list key (see lib/admin/lists-config),
// each shaped as createMany data with a 0-based `order`. Both the seed and the
// reset action rebuild a list by deleteMany() + createMany({ data }).
export const listDefaults: Record<string, Record<string, unknown>[]> = {
  navItems: defaultNavItems.map((n, order) => ({ ...n, order })),
  heroEyebrows: defaultHeroEyebrows.map((text, order) => ({ text, order })),
  manifestoFoots: defaultManifestoFoots.map((m, order) => ({ ...m, order })),
  ...
  policies: defaultPolicies.map((p, order) => ({ ...p, order })),
  footerColumns: defaultFooterColumns.map((c, order) => ({
    title: c.title,
    links: c.links,
    order,
  })),
